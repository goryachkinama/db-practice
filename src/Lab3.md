...
Этот управляющий символ должен использоваться в образце перед трафаретным символом, сообщая о том, 
что последний следует трактовать как обычный символ. 
Например, если в некотором поле следует отыскать все значения, содержащие символ «_», то шаблон ‘%_%’ приведет к тому, 
...
### Подзапросы/вложенные запросы и их виды
...
* Составить по одному запросу на UNION, EXCEPT и INTERSECT
* Составить по одному запросу с подзапросом в выражении WHERE, HAVING, FROM, 
  в качестве спецификации столбца в выражении SELECT, в командах INSERT, UPDATE, DELETE
...
### Ещё примеры вложенных запросов...
