...
4. Создать индексированное представление с ФИО и Датой рождения. Индекс использовать для поля "ФИО".
5. Написать скрипт для реорганизации индекса по полю "ФИО".
6. Сделать реорганизацию всех индексов в таблице.
7. Написать скрипт для перестроения индекса по полю "Дата рождения".
8. Сделать перестроение всех индексов в таблице.
9. Вывести все индексы таблицы и их типы.  
10. Удалить самостоятельный индекс.
11. Удалить индекс, добавленный через ограничение.
...
•	Небольшие  таблицы (меньше 100 строк) не нуждаются в индексах. Процесс индексации такой таблицы достаточно быстр, так что получается достаточно высокая производительность без создания индекса.
...
•	Любое поле, используемое в агрегирующей функции (сумма, агрегат и т.д.), которая содержит предложения GROUP BY или ORDER BY и используется в JOIN, должно рассматриваться как кандидат на индекс. Дело в том, что ядро СУБД для этих операций использует индексы.
