...
4.*Размер фотографии не должен превышать 2 мегабайта (т.е. если мы работаем с PhotoPath, то ввести ограничение на длину строки с путём к файлу).
5. Задать варианты значений для именования пола в таблице с полами, для вариантов цветов тегов через команду CHECK.
...
```sql
ALTER TABLE Employees ADD EmployeeID uniqueidentifier NOT NULL DEFAULT NEWID()
```
